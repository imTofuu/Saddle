...
if [ $rebuild -ne 0 ] ; then
    ...
    for i in $(find src -name \*.cpp) ; do
        name=$(basename $i)
        g++ -fPIC -c -obin/$BINPATH/${name%.*}.o $i -I../dependencies/GLFW/include -I../dependencies/glad/include -I../dependencies/imgui -I../dependencies/imgui/backends -DSDL_PLATFORM_LINUX -DSDL_DEBUG
    done

    echo "Linking into single shared object..."

    g++ -shared -L../bin/$BINPATH -o ../bin/$BINPATH/libSaddle.so $(find bin/$BINPATH -name \*.o) -Wl,--whole-archive -lglad -lglfw3 -limgui -Wl,--no-whole-archive
...
if [ $rebuild -ne 0 ] ; then
    ...
    g++ -o ../bin/$BINPATH/run $(find src -name \*.cpp) -L../bin/$BINPATH -L. -lSaddle -I../Saddle/src -I../dependencies/GLFW/include -DSDL_PLATFORM_LINUX -DSDL_DEBUG
